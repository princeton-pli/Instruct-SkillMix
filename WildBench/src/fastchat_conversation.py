...
def map_to_conv(model_name):
    if "checkpoints" in model_name:
        if "mistral" or "llama2" in model_name:
            conv = get_conv_template("alpaca")
        elif "llama3" in model_name:
            conv = get_conv_template("llama-3-alpaca")
    elif model_name in HF_TEMPLATED_MODELS:
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ...
                    ret += f"<|start_header_id|>{role}<|end_header_id|>\n\n"
            return ret
        elif self.sep_style == SeparatorStyle.LLAMA3_ALPACA:
            ret = "<|begin_of_text|>"
            if self.system_message:
                ret += system_prompt
            else:
            ...
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ret += f"<|start_header_id|>{role}<|end_header_id|>\n\n"
                    if role == "user":
                        message = f"Below is an instruction that describes a task. Write a response that appropriately completes the request.\n\n### Instruction:\n{message.strip()}\n\n### Response:"
                    ret += f"{message.strip()}<|eot_id|>"
                else:
                    ret += f"<|start_header_id|>{role}<|end_header_id|>\n\n"
            return ret
...
register_conv_template(
    Conversation(
        ...
        system_template="<|start_header_id|>system<|end_header_id|>\n\n{system_message}<|eot_id|>",
        roles=("user", "assistant"),
        sep_style=SeparatorStyle.LLAMA3_ALPACA,
        sep="",
        stop_str="<|eot_id|>",
        stop_token_ids=[128001, 128009],
    )
