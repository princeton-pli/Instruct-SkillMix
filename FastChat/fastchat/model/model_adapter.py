...
class ChatGPTAdapter(BaseModelAdapter):
    ...
    def match(self, model_path: str):
        return model_path in OPENAI_MODEL_LIST or model_path in OPENAI_AZURE_MODEL_LIST
...
## ADDED FOR SFT COMPATIBILITY
class SFTModelAdapter(BaseModelAdapter):
    """The model adapter for models we fine-tuned"""
    
    def match(self, model_path: str):
        return "checkpoints" in model_path.lower()
    ...
    def get_default_conv_template(self, model_path: str) -> Conversation:
        if "mistral" in model_path:
            ...
            conv = get_conv_template("llama-3-alpaca")
        else:
            conv = get_conv_template("alpaca")
        return conv
...
register_model_adapter(SFTModelAdapter) ## ADDED FOR SFT COMPATIBILITY
